...
# --- Unpack tester payload (runner/core/deps) -------------------------------
PAYLOAD_TAR="tester_payload.tar"
...
# Reference any DLLs shipped with the tester payload (core/deps).
EXTRA_DLL_REFS=""
for DLL in ./*.dll; do
	[ -f "$DLL" ] || continue
	EXTRA_DLL_REFS="$EXTRA_DLL_REFS -r:$(pwd)/$(basename "$DLL")"
done
...
EXECUTABLE=false
$PROGRAM $PKGDOTNET $NUNITLIB $NETSTANDARDLIB $MICROSOFTLIB $MICROSOFTLIB1 $EXTRA_DLL_REFS $CSharp_DLL_REFERENCE -out:$OUTPUTFILE -lib:/usr/lib/mono @.vpl_source_files &>.vpl_compilation_message
if [ -f $OUTPUTFILE ] ; then
	...
	if [ "$NUNITLIB" != "" ] ; then
		$PROGRAM $PKGDOTNET $NUNITLIB $NETSTANDARDLIB $MICROSOFTLIB $MICROSOFTLIB1 $EXTRA_DLL_REFS -out:$OUTPUTFILE -target:library -lib:/usr/lib/mono @.vpl_source_files &> /dev/null
	fi
fi
rm .vpl_source_files
if [ -f $OUTPUTFILE ] ; then
